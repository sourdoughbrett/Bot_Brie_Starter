'''
-This script contains a sample multi-variate crossover strategy with the MACD and Lazybear Histogram Color.
-This script uses OHLC Bars on a 1-day timeframe
...
strat_name = 'OHLC_Lazybear_Alert' # Strat Name
...
symbols = ['ARKK','TSLA','GRPN','AAPL','MSFT','CSCO','NVDA','TSM','ORCL','PTON',
           'ADBE','ASML','AMD','CRM','SAP','INTC','INTU','IBM','QCOM','AMAT',
           'NOW','SONY','UBER','MU','PANW','INFY','SHOP','SNPS','BBY',
           'FI','CDNS','KLAC','ANET','WDAY','ARM','TEAM','ROP','NXPI','SNOW','DELL',
           'APH','MRVL','FTNT','MSI','ADSK','ON','CRWD','STM','TEL','TTD','PLTR',
           'CTSH','FIS','GFS','DDOG','PYPL','FTV','GLW','ANSS','JPM','BOTZ',
           'MDB','WIT','UBS','KEYS','GIB','ZS','HPE','MPWR','FICO','BR',
           'NET','VRSN','ZM','NOK','GRMN','TDY','UMC','FSLR','SEDG','PTC',
           'JBL','ENPH','TYL','ASX','AKAM','CHKP','NTAP','SWKS','PAYC','TER',
           'ERIC','EPAM','WDC','SMCI','ENTG','APP','STX','DT','GRAB','IOT',
           'TRMB','OKTA','AZPN','SSNC','LDOS','MANH','ZBRA','FLEX','U','LSCC','BILL',
           'GEN','PSTG','LOGI','GDDY','NICE','DOX','PCTY','TOST','PATH',
           'FFIV','OTEX','DBX','SNX','QRVO','PCOR','CFLT','JNPR','UI','KVYO','DOCU',
           'CCCS','GLOB','NTNX','WEX','ESTC','MNDY','OLED','GWRE','CGNX','CACI','GTLB',
           'CIEN','ARW','DUOL','YMM','CYBR','ZI','G','APPF','AFRM','DSGX','LFUS',
           'ALGM','RMBS','FN','INFA','FRSH','MTSI','MKSI','ST','PSN','DLO','UPST',
           'SAIC','AMKR','SMAR','WK','ACLS','WIX','TENB','NSIT','NOVT','ALTR','S','COHR',
           'DV','EXLS','FIVN','MSTR','BRZE','RELY','CLVT','TDC','HCP','AVT','POWI','DXC',
           'BMI','CNXC','BDC','LYFT','CRUS','PYCR','ASAN','ASGN','EEFT','SQSP','LRN','ROKU',
           'NFLX','BABA','BA','MRNA','CHWY','RUN','BROS','ZG','ETSY','EMN','NIO','GME',
           'AMC','MTCH','HOOD','MEDP','ALRM','GO','RNG','COUR','UDMY','MARA','RIOT',
           'TEX','FRPT','AI','HMY','SIG','PSLV','BTG','JOBY','KD','CVNA','ALK','UAL','PTEN',
           'MDGL','WOLF','STNE','MTZ','IGT','TAL','EXP','CART','CTLT','SOFI',
           'SE','BAC','VOO','SPY','VTI','GS','WFC','BLK','SCHW','IWM','QQQ','DIA','VUG','RIVN',
           'X','SQQQ','COIN','LCID','KC','ANF','LULU','AEO','URBN','CROX','MPC','EQR','META',
           'SOUN','VEEV','VIPS','SEAT','TGT','FERG','VSCO','GLD','GDX','DJT','GLUE','SERV',
           'HUSA','ACHR','IQ','JD','LTBR','HKD','PARA','MNPR','FUTU','NFE','PRCH','OKLO','QBTS',
           'BBAI','GRND','TWLO','XYZ','NCLH']
...
def calculate_momentum(df, column="close", bollinger_period=20, keltner_period=20, momentum_period=14):
    ...
    Returns:
        pandas.DataFrame: DataFrame containing the Squeeze Indicator, Momentum Histogram, and Color Coding.
    """
    ...
    df["momentum_histogram"] = df["momentum"].rolling(window=5).mean()  # Smoothed histogram

    # Color Coding for Histogram
    df["momentum_color"] = "gray"  # Default

    # Light Green: Bullish Increasing
    df.loc[(df["momentum_histogram"] > 0) & (df["momentum_histogram"].diff() > 0), "momentum_color"] = "light_green"

    # Dark Green: Bullish Weakening
    df.loc[(df["momentum_histogram"] > 0) & (df["momentum_histogram"].diff() < 0), "momentum_color"] = "dark_green"

    # Light Red: Bearish Increasing
    df.loc[(df["momentum_histogram"] < 0) & (df["momentum_histogram"].diff() < 0), "momentum_color"] = "light_red"

    # Dark Red: Bearish Weakening
    df.loc[(df["momentum_histogram"] < 0) & (df["momentum_histogram"].diff() > 0), "momentum_color"] = "dark_red"
    ...
    momentum_df = pd.DataFrame({
        ...
        "momentum_color": df["momentum_color"]  # Color classification
    }, index=df.index)
...
hist_data_raw['momentum_color'] = momentum_df['momentum_color']
#MACD concat
...
def main():
    ...
    while True:
        ...
        if datezone_est >= market_start_time and datezone_est <= market_end_time:
            ...
            #
            momentum_df = calculate_momentum(df=stock_data, column="close", bollinger_period=20, keltner_period=20, momentum_period=14)
            stock_data["squeeze_on"] = momentum_df["squeeze_on"]
            stock_data['momentum_histogram'] = momentum_df['momentum_histogram']
            stock_data['momentum_color'] = momentum_df['momentum_color']
            ...
            for symbol in symbols:
                ...
                # Momentum Color
                cur_momentum_color = cur_period_data['momentum_color']
                lag1_momentum_color = lag1_period_data['momentum_color']
                # High/Low Bar Data
                ...
                if not existing_long_pos and not existing_short_pos:
                    ...
                    if ((lag1_momentum_color == 'dark_green' and cur_momentum_color == 'light_green') and \
                        (lag1_macd < lag1_macd_signal and cur_macd > cur_macd_signal) and \
                        (cur_rsi > 50)):
                ...
                if not existing_long_pos and not existing_short_pos:
                    ...
                    if (((lag1_momentum_color == 'dark_red' and cur_momentum_color == 'light_red')) and \
                        (lag1_macd > lag1_macd_signal and cur_macd < cur_macd_signal) and \
                        (cur_rsi < 50)):
                    # SHORT TRADE ENTRY LOGIC PLACED HERE
                            try:
