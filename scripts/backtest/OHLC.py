...
timeframe = "Day" # change timeframe to Day or Hour for backtesting if needed
...
#MACD concat
macd_df = calculate_macd(df=hist_data_raw,column='close',fast_period=macd_fast_period_val, slow_period=macd_slow_period_val, signal_period=macd_signal_period_val)
...
sar_df = calculate_parabolic_SAR(df=hist_data_raw, column='close',acceleration=psar_acceleration_dev_factor, maximum=psar_maximum_dev_factor)
hist_data_raw['parabolic_sar'] = sar_df['parabolic_sar']
#Stochastic concat
stochastic_df = calculate_stochastic(df=hist_data_raw, high_col='high',low_col='low',close_col='close',fastk_period=stoch_fastK_period_val,slowk_period=stoch_slowK_period_val, slowd_period=stoch_slowD_period_val)
...
#SMA_Fast Concat
sma_series = calculate_sma_fast(df=hist_data_raw, column='close',period=sma_fast_period_val)
hist_data_raw['SMA_Fast'] = sma_series
#EMA_Fast Concat
ema_series_fast = calculate_ema_fast(df=hist_data_raw, column='close',period=ema_fast_period_val)
hist_data_raw['EMA_Fast'] = ema_series_fast
#EMA_Slow Concat
ema_series_slow = calculate_ema_slow(df=hist_data_raw, column='close',period=ema_slow_period_val)
...
def generate_signals(df):
    ...
    for i in range(0, len(df)):
        ...
        if ((lag1['SlowK'] < lag1['SlowD'] and cur['SlowK'] > cur['SlowD']) and
