...
5.	Please update the following accordingly if errors are produced:
     ...
     - `daily_buying_power` or `buying_power`
     - `API_KEY`
     - `SECRET_KEY`
...
def generate_signals(df):
    ...
    return signals
```
Now you can run the backtest on different timeframes, tp/sl parameters, indicator values, and datetimes to test for efficacy.
